...
export const RegisterSecondStep: FC<IProps> = ({step, setStep}) => {
    ...
    return (
        <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
            ...
            <p className={styles.form__prompt}>{formState.errors.surname && LightText(formState.errors.surname?.message || '', 'Поле не может быть пустым', '', true)}</p>
            {step === 3 ? <input type="submit" className={styles.form__submit} value='ЗАРЕГИСТРИРОВАТЬСЯ'/> :
                <button disabled={!formState.isValid} type='submit' className={formState.isValid ? styles.form__btn : `${styles.form__btn} ${styles.form__btn_error}`}>Последний
                    шаг</button>}
        </form>
    )
}
